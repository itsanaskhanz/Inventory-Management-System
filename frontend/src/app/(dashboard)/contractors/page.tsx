...
const Page = () => {
  ...
  const [isDeleteContractorModelOpen, setIsDeleteContractorModelOpen] = useState(false);
  const [selectedContractorToDelete, setSelectedContractorToDelete] = useState<string | null>(null);

  const { data: response, isLoading, isError } = useGetUsersByRole(UserRole.ADMIN, page, limit);
  ...
  const columns: ColumnDef<User>[] = [
    ...
    },
    {
      ...
      enableSorting: false,
      enableHiding: false,
      cell: ({ row }) => <TableActions onDelete={() => openDeleteModal(row.original.id)} />,
    },
  ];
    ...
    <>
      <div className="flex flex-col gap-6">
        <PageHeader
          ...
          actions={
            <Button size="sm" onClick={() => setIsCreateContractorModalOpen(true)}>
              New Contractor
            </Button>
          }
        />
        ...
        >
          <Table data={users} columns={columns} page={page} setPage={setPage} totalPages={totalPages} />
        </AsyncState>
      </div>
